...
### Uma Profile Import OCR

New uma profiles can be directly imported from screenshots rather than manually typing in stats and skills. The OCR implementation is custom-made for this version of the Umalator and not dependent on other uma importing OCRs. Currently, it has the capability of identifying the specific uma, the statline, and parsing through all the skills (though sometimes requires manual identification). Do note that aptitudes are not currently imported at the moment, since aptitudes are set manually by the desired simulation terrain, distance, and style. 
